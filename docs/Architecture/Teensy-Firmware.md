...
├── StartupHooks.c               Teensyduino core startup-hook overrides (boot-time work that can't wait for setup())
...
Every power-up and reboot (`REBOOT` is a full MCU reset — see [Reboot-Display-Blanking.md](Reboot-Display-Blanking.md)) begins in the Teensyduino core's `ResetHandler` (`cores/teensy4/startup.c`), which brings the chip up — memory, clocks, peripherals — calls `usb_init()` to attach the USB device, deliberately waits until ~300ms after reset (≈280ms of that after `usb_init()`) for the host to enumerate, runs the C++ constructors, and then hands off to `main()` → `setup()`. Along the way the core calls three weak, empty hook functions a sketch can override: `startup_early_hook()`, `startup_middle_hook()`, `startup_late_hook()`. `StartupHooks.c` is where the full firmware defines them; it currently overrides only the middle hook (the last point before `usb_init()`) to run `MidiDevName_AppendUniqueID()`, so the USB MIDI product/serial strings already carry the chip ID the first time the host reads them. See [Constraints.md](Constraints.md#startup-hooks-run-before-the-c-runtime-exists) for what is and isn't usable inside a hook.
...
A host app can query which mode is currently active via `FWCheckToken`. Only a restricted command set (`ResetC64Token`, `LaunchFileToken`, `VersionInfoToken`, `FWCheckToken`) is available while in minimal mode. Dual-boot hex combining (so both images ship in one `.hex`; a TeensyROM+ extension host is never part of it, and ships as its own `.TRH` package) is handled by `tools/build-firmware.mjs` — see [Build-System.md](Build-System.md).
