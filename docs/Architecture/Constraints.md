...
**Local build capability exists to verify this going forward** (ask before assuming it's still set up if picking this up much later): `npm run build:tr-plus -- --no-extensions --skip-minimal-build --skip-combine`, run from the repo root, builds the Fab04 main firmware in about a minute and prints this exact report — use it to check real before/after impact of any `FLASHMEM` migration rather than estimating from line counts. The `--no-extensions` is what pins it to the configuration measured above: since extensions became the TR+ default, the bare recipe also builds the extension image and links the main one with the loader compiled in, which moves every number in that report. Either configuration is a fair baseline as long as before and after use the same one; this doc's reasoning was measured without extensions, so reproducing it needs the flag.
