...
One number in that report never applies to this firmware: `FLASH: … free for files` is `teensy_size`'s whole-board figure, not the slot the main image is actually linked into. With extensions on that slot is 7168K at `0x60060000` — everything up to the extension host slot at `0x60760000` — but the limit that binds first is the updater's, not the linker's: a self-update stages a whole spare copy of the firmware beside the running one, below the 640K the TR+ reserves at the top of flash. On 2026-09-25 the combine step reported `main 1703.0K of 7168K (23.8%)` and the headroom check that follows it `1689.0 K of headroom left before the self-update ceiling` (3776K) — the number to read, not the ~6.4MB the report prints. Both lines only appear in a full `npm run build:tr-plus` (no `--skip-combine`), which prints each region's usage as it combines them.
