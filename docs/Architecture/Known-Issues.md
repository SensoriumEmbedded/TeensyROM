...
**This isn't just a nice-to-have.** Per the user (2026-08-12), the Fab04 (TR+) main-firmware build — the tightest configuration, since it compiles in REU/KernalReplace/Freezers on top of everything else — had, as of that date, only **~9-10KB of RAM1 "padding"** left before hitting a hard link failure (measured: `RAM1: variables:254468, code:220104, padding:9272  free for local variables:40444` bytes). That figure has since moved, and it moves with almost every commit: with the extension loader compiled in, a full `npm run build:tr-plus` measured `RAM1: variables:241380, code:214472, padding:14904   free for local variables:53532` as of the extension-host PR (#37). Across the builds measured during that work, `padding` fell while `free for local variables` held at 53532, so padding is the slack that runs out and the figure to watch. That figure is the tree's own, and it is not reachable by adding up the deltas of the branches merged into it. Measure the tree you are pricing, not the commits that went into it. Without the loader, a 2026-09-23 build measured `padding:19336`; that one has not been re-measured since. Date a measurement to a commit rather than to a day -- a day holds enough growth to invalidate it -- and re-measure before pricing this work off it — see [Constraints.md](Constraints.md#memory-budgets-are-hard-caps-not-soft-targets). RAM1's biggest single consumer, the `RAM_Image` buffer, is fixed and won't shrink — so `FLASHMEM` migrations are the main lever available to keep this build compiling as more features get added, not just an optimization. (Correcting an earlier framing here: MinimalBoot's RAM scarcity is well-documented and obvious, but RAM1 pressure in the full Fab04 build is actually the tighter, less-visible constraint.)
