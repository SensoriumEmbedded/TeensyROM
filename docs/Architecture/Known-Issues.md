...
- `Menu_Regs.i`/`Menu_Regs.h` duplication — `Menu_Regs.i` now generated from `Menu_Regs.h` via `tools/lib/menu-regs.mjs` (formerly `gen_menu_regs_i.py`), run at the start of `npm run build:c64`; verified byte-identical `.prg` output against the old hand-written file
...
**This isn't just a nice-to-have.** Per the user (2026-08-12), the Fab04 (TR+) main-firmware build — the tightest configuration, since it compiles in REU/KernalReplace/Freezers on top of everything else — had, as of that date, only **~9-10KB of RAM1 "padding"** left before hitting a hard link failure (measured: `RAM1: variables:254468, code:220104, padding:9272  free for local variables:40444` bytes). That figure has since moved, and it moves with almost every commit: on this branch at `9c00450` a full `npm run build:tr-plus` measured `RAM1: variables:241380, code:213896, padding:15480   free for local variables:53532` with the extension loader compiled in, where a build earlier on 2026-09-23 measured `padding:16056` -- the review commits landed in between spent 576 of those bytes. Note which number absorbed them: `free for local variables` held at 53532 while `padding` fell, so padding is the slack that runs out and the figure to watch. At `66e8424` the same build measured `RAM1: variables:241380, code:214472, padding:14904   free for local variables:53532` -- 576 fewer bytes of padding than at `9c00450`, and again `free for local variables` did not move. That figure is the tree's own, and it is not reachable by adding up the deltas of the branches merged into it: reaching for it that way produced a wrong answer twice before anyone built the tree. Measure the tree you are pricing, not the commits that went into it. Without the loader, a 2026-09-23 build measured `padding:19336`; that one has not been re-measured since. Date a measurement to a commit rather than to a day -- a day holds enough growth to invalidate it -- and re-measure before pricing this work off it — see [Constraints.md](Constraints.md#memory-budgets-are-hard-caps-not-soft-targets). RAM1's biggest single consumer, the `RAM_Image` buffer, is fixed and won't shrink — so `FLASHMEM` migrations are the main lever available to keep this build compiling as more features get added, not just an optimization. (Correcting an earlier framing here: MinimalBoot's RAM scarcity is well-documented and obvious, but RAM1 pressure in the full Fab04 build is actually the tighter, less-visible constraint.)
...
- [ ] `PollingHndlr_TR_BASIC` — `Source/Teensy/MinimalBoot/Common/IO_Handlers/IOH_TR_BASIC.c` — main-loop-only (`PollingHndlr`), like `PollingHndlr_REU` which already has `FLASHMEM`.
- [ ] `PollingHndlr_TeensyROM` — `Source/Teensy/MinimalBoot/Common/IO_Handlers/IOH_TeensyROM.c:1018` — same pattern, main-loop-only. (Note: most of what `StatusFunction[]` dispatches, plus their exclusive helpers, were split into `StatusFunctions.c` since this was written, and everything that moved carries `FLASHMEM`. Three dispatch targets did not move and carry none: `MenuChange` in `DriveDirLoad.ino`, and `IOHandlerSelectInit` and `IOHandlerNextInit` in `IOHandlers.ino`. They are reached only through this same main-loop dispatch, so they look like candidates, but each still needs the per-function call-graph check every other entry here got. `PollingHndlr_TeensyROM` itself stayed in `IOH_TeensyROM.c` and is still missing it.)
...
- `Min_DriveDirLoad.ino` (MinimalBoot) — the minimal image defines its **own** `SendMsgPrintfln`/`SendMsgPrintf`, separate from the full firmware's in `FileParsers.ino` above, and it had the same unbounded `vsprintf` into a local `SerialStringBuf[MaxPathLength]`. (The full firmware's `DriveDirLoad.ino` was listed here too, as a third copy. It is not one: it only *calls* the formatter and defines none — checked at this branch's base as well, where it holds no `vsprintf` either. What is genuinely duplicated between the two builds is `LoadFile()`/`ParseCRTHeader()`, not the formatter.) That call chain is a **more severe variant of the FileParsers.ino case above**: `LoadFile()` declares `uint8_t lclBuf[CRT_MAIN_HDR_LEN]` (64 bytes, confirmed via `DriveDirLoad.h:170`), and the CRT "Name" field read by `SendMsgPrintfln("Name: %s", (CRT_Image+0x20))` starts at byte 32 and is a 32-byte field ending *exactly* at byte 64 — the last byte of `lclBuf` itself. An unterminated Name field (fills all 32 bytes, no padding) causes the `%s` read to run off the end of `lclBuf` immediately — a stack **out-of-bounds read at the source**, before even reaching the already-known destination-buffer overflow. Tighter than the `FileParsers.ino` instance, where the Name field sits inside a buffer holding the whole loaded file (an unterminated name there just reads into subsequent legitimate file data, not immediately out of bounds). Present in both builds since `Source/C64`-style code sharing doesn't apply here — each `LoadFile()` is its own independent implementation. **Fixed**: `vsnprintf` in the minimal image's copy (`Min_DriveDirLoad.ino:417`), and `%.32s` at both `Name:` call sites (`FileParsers.ino:93`, `Min_DriveDirLoad.ino:272`). All three are pinned by `Source/Teensy/tests/c64-message-buffer.test.js`, which also fails if either formatter goes back to `vsprintf`.
...
**Where:** the `LSFileName[FNCount++]` and `RAM_Image[StreamOffsetAddr++]` writes in `Source/Teensy/MinimalBoot/Common/IO_Handlers/IOH_TR_BASIC.c`, inside `IO1Hndlr_TR_BASIC` (called directly from `isrPHI2`, `ISRs.c:128`).
...
**Where:** `Source/Teensy/MinimalBoot/Common/IO_Handlers/IOH_TR_BASIC.c`, inside `IO1Hndlr_TR_BASIC` (ISR-called, `TR_BASDataReg` write case — the TPUT BASIC command's output path).
...
2. A generator, **`tools/lib/menu-regs.mjs`** (a Node port of the original `gen_menu_regs_i.py`), parses that region directly and emits ACME assignment lines — no C preprocessor involved. (The original plan called for `gcc -E`/`cpp`, but the synced region turned out to contain zero `#ifdef`/`#include` for a preprocessor to resolve, and a plain macro-expansion pass can't translate C `enum { A = 1, B };` block syntax into ACME's flat `A = 1` / `B = 2` assignment lines anyway — that translation is what the script does.) Handles multi-line enums, `#define` constants, auto-incrementing members with no explicit value, and comment-style translation (`//` → `;`).
...
6. Regeneration is the first step of **`tools/build-c64.mjs`**, so every C64 build regenerates `Menu_Regs.i` from `Menu_Regs.h` before assembling.
