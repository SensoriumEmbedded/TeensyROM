...
**This isn't just a nice-to-have.** Per the user (2026-08-12), the Fab04 (TR+) main-firmware build — the tightest configuration, since it compiles in REU/KernalReplace/Freezers on top of everything else — had, as of that date, only **~9-10KB of RAM1 "padding"** left before hitting a hard link failure (measured: `RAM1: variables:254468, code:220104, padding:9272  free for local variables:40444` bytes). That figure has since moved, and it moves with almost every commit: on this branch at `9c00450` a full `npm run build:tr-plus` measured `RAM1: variables:241380, code:213896, padding:15480   free for local variables:53532` with the extension loader compiled in, where a build earlier on 2026-09-23 measured `padding:16056` -- the review commits landed in between spent 576 of those bytes. Note which number absorbed them: `free for local variables` held at 53532 while `padding` fell, so padding is the slack that runs out and the figure to watch. At `66e8424`, with the 832ada5 panel's two lanes among the commits in it, the same build measured `RAM1: variables:241380, code:214472, padding:14904   free for local variables:53532` -- 576 fewer bytes of padding than at `9c00450`, and again `free for local variables` did not move. Note that this figure is not derivable from the lanes' own numbers. Measured against `832ada5` (`FLASH: code:267084`, `RAM1: code:214216, padding:15160`), the Auditor tip `d743e9c` is -72 FLASH code and leaves RAM1 untouched (`padding:15160`, which is the baseline's own figure); the Adversary tip `fc9ed66` -- stacked on `d743e9c`, not parallel to it -- is +64 FLASH code and `padding:14968`. The merged tree at `66e8424` is +216 FLASH code and `padding:14904`, matching neither lane: `832ada5..66e8424` is 23 commits, the two lanes are 9 of them, and 6 of the other 14 touch firmware source. Measure the tree you are pricing, not the commits that went into it. Without the loader, a 2026-09-23 build measured `padding:19336`; that one has not been re-measured since. Date a measurement to a commit rather than to a day -- a day holds enough growth to invalidate it -- and re-measure before pricing this work off it — see [Constraints.md](Constraints.md#memory-budgets-are-hard-caps-not-soft-targets). RAM1's biggest single consumer, the `RAM_Image` buffer, is fixed and won't shrink — so `FLASHMEM` migrations are the main lever available to keep this build compiling as more features get added, not just an optimization. (Correcting an earlier framing here: MinimalBoot's RAM scarcity is well-documented and obvious, but RAM1 pressure in the full Fab04 build is actually the tighter, less-visible constraint.)
...
- [ ] `PollingHndlr_TeensyROM` — `Source/Teensy/MinimalBoot/Common/IO_Handlers/IOH_TeensyROM.c:1018` — same pattern, main-loop-only. (Note: the 34 functions `StatusFunction[]` dispatches, plus their exclusive helpers, were split into `StatusFunctions.c` since this was written; all 39 of those already have `FLASHMEM`. `PollingHndlr_TeensyROM` itself stayed in `IOH_TeensyROM.c` and is still missing it.)
