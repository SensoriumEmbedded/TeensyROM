...
Page files (index → content, per the FW 0.8 layout): `Pg_Index.asm`, `Pg_TRSettings.asm` (TeensyROM General), `Pg_StartupOptions.asm`, `Pg_ColorConfig.asm` (Menu Colors), `Pg_MIDISettings.asm` (MIDI Message Filters), `Pg_EthernetSettings.asm`, `Pg_TimeRTCSettings.asm`, `Pg_InfoOther.asm` (Info: General), `Pg_InfoHotKey.asm` (Info: HotKeys), `Pg_InstalledExt.asm` (Installed Extensions — placeholder banner, no slot data yet).
