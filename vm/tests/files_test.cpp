...
int main(int argc,char **argv){
 ...
 assert(!openFlags(nullptr,7,&i));  //a module that hands over a null path is refused, not followed
 ...
 assert(readFile(h,100,data,1)<0);  //SdFat refuses a seek past EOF: an error, not a short read
 ...
 r={VmFsOp::Rename,0,0,0,"/VMS/DOSVM/D/Y.TXT",nullptr};assert(fileOp(&r)<0);
 r={VmFsOp::Mkdir,0,0,0,nullptr,nullptr};assert(fileOp(&r)<0);
 r={(VmFsOp)99,0,0,0,nullptr,nullptr};assert(fileOp(&r)<0);
 ...
 puts("PASS: actual generic host file services; read/write/flush/truncate/rename/list/space, 24 handles, null and invalid paths/flags/spans, reads past EOF, unknown operations and injected write/flush failures");
}
