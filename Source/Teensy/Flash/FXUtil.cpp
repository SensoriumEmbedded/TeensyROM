...
//******************************************************************************
// This file intentionally excludes the Intel HEX hardening added in commit
// 0f8266a (larger 524/256-byte line/data buffers, CRC32 verification of the
// transferred file, overflow-safe address bounds checking, exact hex
// line-length/nibble validation, and detection of trailing data after the
// EOF record). That hardening cost ~1.4KB of RAM1 (768 bytes ITCM code +
// 640 bytes static DTCM data, measured via an isolated A/B build) that is
// needed elsewhere on this platform. Reverting it means update_firmware()
// is back to trusting well-formed input: a malformed or truncated .hex file
// on the update media can be parsed further than it should before failing,
// and there's no way to confirm the transferred file matches what was
// staged. The full hardened implementation (and its test suite at
// Source/Teensy/tests/flash-update-parser.cpp) is preserved in git history
// at commit 0f8266a if this trade needs to be revisited.
#include <Arduino.h>
extern "C" {
