...
if not result.rebooted:
    ...
    print('tryLaunch (Common/VMRegistry.h) declines eight ways -- bad client, missing')
    print('package, ambiguous registry, failed preflight, no host installed, host ABI,')
    print('host services, path too long -- and a path that is not an extension at all just')
    print('falls through to an ordinary launch. Every one of those reports through')
